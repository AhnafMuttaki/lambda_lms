# Prompt for Copilot: Enrollment Service for LMS

## Context
You are to generate a production-ready enrollment-service for a Learning Management System (LMS) similar to Udemy.
...
### Functional
- Enroll a user in a course (free/paid)
- Get all enrollments for a user
- Update progress for an enrollment (by module/section)
...
- Database migration scripts for MySQL (enrollments, progress tables)
...
- `migrations/001_create_enrollments.sql` (migration script)
- `.env` and `example.env` (all config variables)
- `__tests__/enrollment-api.test.ts` (unit tests for all endpoints)
...
Generate all required files and code for the enrollment-service as described above. 