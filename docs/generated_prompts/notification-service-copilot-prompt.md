# Prompt for Copilot: Notification Service for LMS

## Context
You are to generate a production-ready notification-service for a Learning Management System (LMS) similar to Udemy.
...
### Functional
- Send notifications (email, in-app) to users
- Get all notifications for a user
- Store notification metadata and status
...
- Database migration scripts for MySQL (notifications table)
...
- `migrations/001_create_notifications.sql` (migration script)
- `.env` and `example.env` (all config variables)
- `__tests__/notification-api.test.ts` (unit tests for all endpoints)
...
Generate all required files and code for the notification-service as described above. 